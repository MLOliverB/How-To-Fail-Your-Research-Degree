...
conn.execute("CREATE TABLE IF NOT EXISTS Events(id INT PRIMARY KEY, stage INT, image TEXT, title TEXT, description TEXT, save_condition TEXT, requirement TEXT, effect TEXT)")

 

### Inserting Data
add_activity = "INSERT INTO Activities(id,stage,number,image,title,description,placement) VALUES (?,?,?,?,?,?,?)"
add_event = "INSERT INTO Events(id,stage,image,title,description,save_condition,requirement,effect) VALUES (?,?,?,?,?,?,?,?)"
