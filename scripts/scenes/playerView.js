import { CardBox, AddCardBox, CardDiscardBox, ToolbarButton, nextHandler, startHandler, loadTeam, unloadTeam, pickUpCard } from "../activity_cards/GameBoard.js";
...
export default class playerView extends Phaser.Scene {
    ...
    create() {
		...
		this.toolbarNext.button.disableInteractive();
		this.toolbarStart = new ToolbarButton(this, 0.7, 0.12, "Start Timer", startHandler, undefined, undefined);	// button to start the game
        ...
        for (let s = 1; s < 5; s++) {
            loadActivityCardStack(s, (cards) => {
                ...
                if (s == 4) { // Once all stacks have been loaded, enable the game to be played (by pressing the timer start button)
                    this.currentCardBox.setInteractive();
                }
            });
        }
		...
		for (let i = 0; i < this.numberOfTeams; i++) {
			...
			let team = new Map([
				...
				["cards", cards],			// a 2D array of stages of card boxes, e.g. cards[0] will return the array of card boxes used in the first stage
